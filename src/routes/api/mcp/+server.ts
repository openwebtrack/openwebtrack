...
const authenticateMcpKey = async (rawKey: string | undefined) => {
	...
	db.update(mcpKey)
		...
		.where(eq(mcpKey.id, key.id))
		.catch(() => {});
	return key.userId;
};

const listWebsites = (userId: string) =>
	db
		.selectDistinct({
			id: website.id,
			...
			access: sql<'owner' | 'shared'>`CASE WHEN ${website.userId} = ${userId} THEN 'owner' ELSE 'shared' END`
		})
		.from(website)
		.leftJoin(teamMember, eq(teamMember.websiteId, website.id))
		.where(or(eq(website.userId, userId), eq(teamMember.userId, userId)))
		.orderBy(website.domain);
	...
	createMcpHandler(
		(server) => {
			server.registerTool(
				...
				{
					title: 'List websites',
					description: 'List every website available to this account, including sites it owns and sites shared with it.',
					inputSchema: {},
			...
			server.registerTool(
				'analytics_overview',
				{
					...
					description: 'Get headline traffic, engagement, revenue, and top-page analytics for one accessible website.',
					inputSchema: { websiteId: websiteIdSchema, startDate: z.string().optional(), endDate: z.string().optional() },
					annotations: { readOnlyHint: true }
				},
				async ({ websiteId, startDate, endDate }, extra) => {
					try {
						...
						const [pageviews, sessions, visitors, revenue, topPages] = await Promise.all([
							db.select({ value: count() }).from(pageview).where(pageWhere),
							db.select({ value: count() }).from(analyticsSession).where(sessionWhere),
							db
								.select({ value: sql<number>`COUNT(DISTINCT ${analyticsSession.visitorId})` })
								.from(analyticsSession)
								.where(sessionWhere),
							db
								.select({ value: sql<number>`COALESCE(SUM(${payment.amount}), 0)` })
								.from(payment)
								.where(paymentWhere),
							db.select({ pathname: pageview.pathname, pageviews: count() }).from(pageview).where(pageWhere).groupBy(pageview.pathname).orderBy(desc(count())).limit(10)
						]);
						return textResult({
							website: { id: site.id, domain: site.domain },
							...
							uniqueVisitors: Number(visitors[0]?.value ?? 0),
							revenue: Number(revenue[0]?.value ?? 0),
							currency: site.currency,
							topPages
						});
					} catch (error) {
						return textResult({ error: error instanceof Error ? error.message : 'Tool call failed' }, true);
					}
				}
			);
			...
			server.registerTool(
				'analytics_breakdown',
				{
					...
					description: 'Break down one accessible website by pages, referrers, countries, browsers, devices, event names, or campaigns.',
					inputSchema: {
						websiteId: websiteIdSchema,
						dimension: z.enum(['pages', 'referrers', 'countries', 'browsers', 'devices', 'event_names', 'campaigns']),
				...
				async ({ websiteId, dimension, startDate, endDate, limit }, extra) => {
					try {
						const site = await withWebsite(extra.authInfo!.clientId, websiteId);
						const range = dateRange(startDate, endDate, site.timezone);
						const take = toolLimit(limit);
						const sessionWhere = and(eq(analyticsSession.websiteId, site.id), gte(analyticsSession.startedAt, range.start), lte(analyticsSession.startedAt, range.end));
						const pageWhere = and(eq(pageview.websiteId, site.id), gte(pageview.timestamp, range.start), lte(pageview.timestamp, range.end));
						...
						if (dimension === 'pages')
							return textResult(
								await db.select({ label: pageview.pathname, value: count() }).from(pageview).where(pageWhere).groupBy(pageview.pathname).orderBy(desc(count())).limit(take)
							);
						if (dimension === 'referrers')
							return textResult(
								await db
									.select({ label: sql<string>`COALESCE(${analyticsSession.referrer}, 'Direct')`, value: count() })
									.from(analyticsSession)
									.where(sessionWhere)
						...
						if (dimension === 'countries')
							return textResult(
								await db
									.select({ label: sql<string>`COALESCE(${analyticsSession.country}, 'Unknown')`, value: count() })
									.from(analyticsSession)
									.where(sessionWhere)
						...
						if (dimension === 'browsers')
							return textResult(
								await db
									.select({ label: sql<string>`COALESCE(${analyticsSession.browser}, 'Unknown')`, value: count() })
									.from(analyticsSession)
									.where(sessionWhere)
						...
						if (dimension === 'devices')
							return textResult(
								await db
									.select({ label: sql<string>`COALESCE(${analyticsSession.deviceType}, 'Unknown')`, value: count() })
									.from(analyticsSession)
									.where(sessionWhere)
						...
						if (dimension === 'event_names')
							return textResult(
								await db
									.select({ label: sql<string>`COALESCE(${analyticsEvent.name}, ${analyticsEvent.type})`, value: count() })
									.from(analyticsEvent)
									.where(eventWhere)
									.groupBy(analyticsEvent.name, analyticsEvent.type)
									.orderBy(desc(count()))
									.limit(take)
							);
						return textResult(
							await db
								.select({ label: sql<string>`COALESCE(${analyticsSession.utmCampaign}, 'None')`, value: count() })
								.from(analyticsSession)
								.where(sessionWhere)
								.groupBy(analyticsSession.utmCampaign)
								.orderBy(desc(count()))
								.limit(take)
						);
					} catch (error) {
			...
			server.registerTool(
				'analytics_records',
				{
					...
					description: 'Retrieve recent records from one accessible website. Supports pageviews, events, sessions, payments, and visitors; results are capped at 100.',
					inputSchema: {
						websiteId: websiteIdSchema,
						dataset: z.enum(['pageviews', 'events', 'sessions', 'payments', 'visitors']),
						startDate: z.string().optional(),
						endDate: z.string().optional(),
						limit: z.number().int().min(1).max(100).optional()
					},
				...
				async ({ websiteId, dataset, startDate, endDate, limit }, extra) => {
					try {
						const site = await withWebsite(extra.authInfo!.clientId, websiteId);
						const range = dateRange(startDate, endDate, site.timezone);
						const take = toolLimit(limit);
						...
						if (dataset === 'pageviews')
							return textResult(
								await db
									.select({ url: pageview.url, pathname: pageview.pathname, title: pageview.title, referrer: pageview.referrer, timestamp: pageview.timestamp })
									.from(pageview)
									.where(pageWhere)
						...
						if (dataset === 'events')
							return textResult(
								await db
									.select({ type: analyticsEvent.type, name: analyticsEvent.name, data: analyticsEvent.data, timestamp: analyticsEvent.timestamp })
									.from(analyticsEvent)
									.where(eventWhere)
						...
						if (dataset === 'payments')
							return textResult(
								await db
									.select({ amount: payment.amount, currency: payment.currency, transactionId: payment.transactionId, timestamp: payment.timestamp })
									.from(payment)
									.where(paymentWhere)
						...
						return textResult(
							await db
								.select({ name: visitor.name, isCustomer: visitor.isCustomer, firstSeen: visitor.firstSeen, lastSeen: visitor.lastSeen })
								.from(visitor)
