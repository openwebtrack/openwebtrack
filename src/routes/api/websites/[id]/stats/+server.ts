...
export const GET: RequestHandler = async ({ locals, params, url }) => {
	...
	const [
		...
		db
			...
			.from(analyticsEvent)
			.where(and(eq(analyticsEvent.websiteId, site.id), eq(analyticsEvent.type, 'exit_link'), gte(analyticsEvent.timestamp, start), lte(analyticsEvent.timestamp, end), ...eventConditions))
		...
		db
			...
			.from(analyticsEvent)
			.where(and(eq(analyticsEvent.websiteId, site.id), gte(analyticsEvent.timestamp, start), lte(analyticsEvent.timestamp, end), sql`${analyticsEvent.type} != 'exit_link'`, ...eventConditions))
