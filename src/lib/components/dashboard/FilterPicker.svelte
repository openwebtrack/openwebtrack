...
<script lang="ts">
	...
	const generateOptionsFromVisitors = (type: FilterType): { label: string; value: number }[] => {
		...
		for (const v of visitors) {
			...
			switch (type) {
				...
				case 'device':
					...
					break;
				case 'referrer':
					value = v.source === 'Direct' ? 'Direct' : v.source;
					break;
			}
			if (value) {
	...
	const fetchOptions = async (type: FilterType) => {
		...
		// Generate options from visitors data in demo mode
		if (isDemo) {
			options = generateOptionsFromVisitors(type);
			return;
		}
