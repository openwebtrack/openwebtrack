...
interface TrackingPayload {
	websiteId: string;
	domain: string;
	type: 'pageview' | 'custom' | 'payment' | 'exit_link';
	href: string;
...
((w: TrackerWindow) => {
	...
	/**
	 * Retrieves or creates an ephemeral ID stored in sessionStorage only.
	 * No cookies are written. IDs are anonymous and reset on every new browser session.
	 */
	const resolveSessionId = (storageKey: string): string => {
		try {
	...
	const createPayload = (): TrackingPayload | null => {
		try {
			...
			return {
				...
				visitorId: resolveSessionId(STORAGE_VISITOR),
				sessionId: resolveSessionId(STORAGE_SESSION),
	...
	const processExternalLink = (a: HTMLAnchorElement | null) => {
		...
		try {
			...
			if (payload) {
				...
				payload.data = cleanseData({ url: a.href, text: a.textContent?.trim() || '' });
				transmit(payload);
			}
		} catch {}
	};
